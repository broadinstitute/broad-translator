...
query <- rjson::fromJSON(json_string)
...
if(nrow(input_meas) > 0){
  for(i in 1:nrow(input_meas)){
    meas_obs[[input_meas$variableID[i]]] <- input_meas$variableValue[i]
  }
}
...
for(i in 1:length(uGroup)){
  ...
  for(j in 1:nrow(os_sub)){
    ...
    if(os_sub$variableGroupID == "GeneExpression"){
      names(var_dist) <- c("DN", "NC", "UP")[as.numeric(names(var_dist))]
    } else if (os_sub$variableGroupID == "GeneKnockdown") {
      names(var_dist) <- dimnames(meas)[[3]][as.numeric(names(var_dist))]
    }
